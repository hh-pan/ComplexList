 ...
 * Desc:
 */
public class RecommandTitle extends AdapterDelegate<List<ItemBean>> {

    private LayoutInflater inflater;

    public RecommandTitle(Context context) {
        inflater = LayoutInflater.from(context);
    }

    @Override
    protected boolean isForViewType(@NonNull List<ItemBean> items, int position) {
        return items.get(position).type == 3;
    }

    @NonNull
    @Override
    protected RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent) {
        return new MyViewHolder(inflater.inflate(R.layout.item_rec_title,parent,false));
    }
